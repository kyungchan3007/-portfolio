...
FMS는 여러 업무 도메인이 하나의 서비스 안에서 연결되어 있어, 인증 상태와 초기 진입 기준이 화면마다 달라지면 접근 제어와 초기 렌더링 품질이 함께 흔들릴 수 있었습니다. 그래서 서버에서 먼저 인증을 판단하고, 렌더링 책임을 분리하는 일반적인 SSR 패턴을 사용했습니다.

핵심은 개별 화면마다 인증 로직을 따로 만들지 않고, 도메인이 늘어나도 같은 인증·초기 렌더링 기준을 유지할 수 있는 공통 토대를 만드는 것이었습니다. 이 패턴을 사용했고, 도메인 특성에 맞게 재구성해 적용했습니다.

예시 코드입니다. 일반적인 패턴을 기반으로, 도메인 특성에 맞게 재구성해 적용했습니다.

```tsx title="server-auth-pattern.tsx"
...
```ts title="contract-validation-pattern.ts"
...
실제로는 각 도메인에 필요한 계약 규칙과 비즈니스 검증을 실행했고, 검증이 끝난 결과만 화면과 상태 관리 레이어로 넘겼습니다.
...
```ts title="design-token-pattern.ts"
export const tokens = {
  color: {
    primary: '#000000',
    surface: '#ffffff',
...
실제로는 도메인에 필요한 UI 규칙과 표현 기준만 공용화했고, 비즈니스 로직은 각 기능 단위 내부에 남겨 두었습니다.

### 5. 테스트 기반 개발과 검증 흐름

기능과 도메인이 빠르게 늘어나는 상황에서는 구현 속도만으로는 품질을 지키기 어려웠습니다. 그래서 핵심 로직은 테스트를 먼저 생각하는 방식으로 진행하고, 구현 이후에는 Vitest와 Playwright 검증 흐름까지 연결해 회귀 위험을 줄였습니다.
...
실제로는 도메인 규칙을 먼저 테스트로 확인한 뒤 구현을 진행했고, 변경 이후에는 관련 단위 테스트와 주요 사용자 흐름 검증을 함께 통과시키는 방식으로 운영했습니다.
