...
| 실시간 처리 | 1분 단위 데이터 전환 시 네트워크와 메인 스레드 병목 발생 | Delta Update + Web Worker 분리 | 네트워크 전송량 **약 30% 감소**, 화면 반영 **3~5초 → 2초** |
...
## 2. Delta Update·Web Worker — 전송량 30%↓, 반영 3~5초→2초
...
- **성과** — 네트워크 전송량 약 30% 감소, 화면 반영 3~5초 → 2초, 메인 스레드 점유 최소화
