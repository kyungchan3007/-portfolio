...
| 항목 | 문제 | 적용 | 결과 |
|---|---|---|---|
| MQTT 멱등성 | 동일 메시지가 반복 저장되어 제어 지연 누적 | Lambda 중간 계층 + `messageId` 기준 멱등성 검증 | 제어 지연 **5초 → 1초 이내** |
| WebSocket 연결 구조 | 화면마다 소켓과 Listener를 개별 생성 | 공통 소켓 모듈과 연결 상태 관리 일원화 | 소켓 생성 지점 일원화 |
| 메시지 수신 안정성 | 중복 구독과 잘못된 cleanup으로 메시지 누락 발생 | 화면 단위 등록·해제 기준 정리 | 수신 누락 제거, 실시간 반영 지연 **50% 이상 개선** |
| 구조 분리 | 실시간·일반 조회·소켓 로직이 전역에 분산돼 기능 변경 시 여러 계층 동시 수정 | 기능 단위 Slice로 분리하는 VSA 구조 전환 | 수정 영향 범위 축소 |
...
- **성과** — 중복 처리·불필요한 후속 전달 제거, 장비 제어 지연 5초 → 1초 이내
...
## 2. WebSocket 연결 일원화 — 중복 구독·수신 누락 제거
...
- **성과** — 소켓 생성 지점 일원화, 중복 구독·수신 누락 제거, 중복 메시지 환경의 실시간 반영 지연 50% 이상 개선, 화면 전환 시 재연결 대기 제거
...
- **성과** — 수정 영향 범위 축소, 재사용성·유지보수성 개선, 장애 원인 추적 시간 단축, 리뷰 범위 명확화, 공통 Socket 변경 지점 일원화
