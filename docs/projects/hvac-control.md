...
| 메시지 수신 안정성 | 중복 구독과 잘못된 cleanup으로 메시지 누락 발생 | 화면 단위 등록·해제 기준 정리 | 수신 성공률 **15% → 100%**, 실시간 반영 지연 **50% 이상 개선** |
...
- **성과** — 소켓 생성 지점 3개 → 1개(67% 축소), 동일 테스트 기준 수신 성공률 15% → 100%, 중복 메시지 환경의 실시간 반영 지연 50% 이상 개선, 화면 전환 시 재연결 대기 제거
