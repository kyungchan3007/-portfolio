...
const PROJECTS: Project[] = [
  {
    ...
    problem: '도메인이 확장되면서 요구사항, API, 검증 기준이 단계마다 달라졌습니다. 구현 방식이 사람마다 달라지고 반복적인 협의와 QA 비용도 커졌습니다.',
...
return validateAndReport(result, intent.acceptanceCriteria);`,
  ...
  },
  {
    ...
    technologies: [
      { name: 'Web Worker', why: ['실시간 대량 데이터를 메인 스레드에서 수신·가공하면서 차트 렌더링과 사용자 인터랙션이 지연됐습니다.', '특히 냉방 수요가 집중되는 시기에 데이터 처리량이 늘어 문제가 두드러졌습니다.'], how: ['실시간 폴링은 부하가 커 15분 주기로 당일 전체를 조회하고, 대용량 응답을 CSV로 받아 fetch ReadableStream 청크로 Web Worker가 수신·보유했습니다.', '화면에 필요한 최근 3시간·약 1,000포인트만 메인 스레드로 전달해 렌더링 부하의 상한을 고정했습니다.'] },
...
requestAnimationFrame(() => applyPatch(nextPatch));`,
    ...
    improvements: {
      intro: '대용량 데이터가 집중되는 운영 조건을 재현하고, 화면의 역할에 따라 데이터 범위와 보관 방식을 나누면서 클라이언트 처리 구조와 협업 기준을 단계적으로 개선했습니다.',
      items: [
        { title: '피크 부하를 직접 재현해 클라이언트 최적화의 한계를 확인했습니다', points: ['전력 사용량이 높은 오후 1~4시와 누적 데이터가 집중되는 오후 9시~자정을 부하 구간으로 정해 데이터 유입을 재현하고 성능을 측정했습니다.', '과거 구간 선택 시 Worker가 보유 데이터에서 추출하고, 시간대 전환 시 이전 구간을 폐기해 브라우저 보관량을 제한했습니다.', '메인 렌더링 부하는 낮췄지만, 데이터가 집중되는 피크 구간에는 약 2초의 반응 지연이 지속됐습니다.', '피크 구간을 재현해 측정한 결과, 클라이언트 연산 분리만으로는 초기 LCP와 피크 시간대 반응성 개선에 한계가 있어 Next.js SSR과 API 분리 구조로 전환했습니다.'] },
...
const SIDE_PROJECTS: Project[] = [
  {
    ...
    technologies: [
      ...
      { name: '상태 4계층 분리', why: ['공유 조건·임시 입력·서버 데이터가 섞이면 캐시 정합성이 깨지고 UI 상태가 서버 데이터에 영향을 줄 수 있었습니다.', '상태를 성격별로 나눠야 각 계층의 책임과 캐시를 정확히 관리할 수 있다고 판단했습니다.'], how: ['명시 조건은 URL, UI 상태는 Zustand, 서버 조회·캐시는 TanStack Query, 인증·검증·서버 캐시는 BFF로 분리했습니다.', '필터는 draft와 applied로 나누고 Query Key 팩토리에서 정규화해 같은 조건이 같은 캐시를 사용하도록 했습니다.'] },
